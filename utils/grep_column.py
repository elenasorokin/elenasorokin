...
if len(sys.argv) == 1:
    print '''\n%s\n A grep-like function. Takes a file that's a list to find in another file, specifying the column index to search in the targetfile. Ex: python grep_column.py listfile.txt targetfile.txt 0. Zero-indexed, whitespace delimited. %s ''' (% '#' * 20, '#' * 20)
    sys.exit(1)
