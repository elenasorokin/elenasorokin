# R functions for power calculations of eQTL studies, under varying MAF and beta parameters. 
...
# Uses formulae for power calculation presented in Methods of 
# https://journals.plos.org/plosgenetics/article?id=10.1371/journal.pgen.1008045#sec008
...
# Parameters:
# n		=    Sample size 
# beta	= Effect size of the SNP on the trait, in SD units. Only square of beta is used, so result is symmetric in +ve and -ve beta values. 
# For simplicity, preferable to have all beta to be +ve.
...
# Use as: source("power_eqtl.R")

# Tested under R version 3.6.3.
...
quantile = qnorm(p = alpha, lower.tail = FALSE) 

pow = pchisq(q = th, df = 1, lower.tail = F, ncp = ncp);
...
pow <- power_beta_maf(beta = (1:30)/10, maf = (1:10)/20, r2 = 1, n = 50, pval = 5e-6)
m <- melt(pow)
...
ggplot(m, aes(x = beta, y = maf, fill = power, label = round(power, digits = 2))) + geom_tile(aes(fill = power)) + 
  ...
  labs (x = "beta (standardized)", y = "Minor allele frequency", title =  bquote(paste(alpha, "=1e-5; n=50", sep='')))  + 
  scale_x_continuous (breaks = unique(m$beta)) + scale_y_continuous (breaks = unique(m$maf))
ggsave(plot = last_plot(), filename = '/home/sorokin/projects/power/power_calcs_eQTLstudy_n50.png',dpi = 330, units = "in", height = 5, width = 10)
